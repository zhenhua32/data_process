def calculate_damage(
    ...
    抗性区=1-抗性/2         抗性≤0
            ...
            =1/(4*抗性+1)    抗性≥75%
    """
    # 在基础攻击力改变的情况下不太好算, 比如火神就会改变基础攻击力.
    # 不过这些都能体现在面板里, 只要在造成伤害的瞬间查看面板即可
    攻击区 = 面板攻击力 + 基础攻击力 * 额外攻击力加成百分比 + 额外攻击力
    暴伤区 = 1 + 暴伤
    暴击期望 = 1 + 暴击率 * 暴伤
    元素精通系数 = 元素精通 * 2.785 / (1404.5 + 元素精通)
    ...
    综合期望伤害 = 非暴击伤害 * 暴击期望
    print(f"{攻击区:.1f} * {暴伤区:.3f} * {增幅区:.3f} * {增伤区:.3f} * {倍率区:.3f} * {防御区:.3f} * {抗性区:.3f} * {独立乘区:.2f} = {暴击伤害}")
